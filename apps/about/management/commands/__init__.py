...
from apps.about.models import CooperativeInfo, CooperativeTimeline, CooperativeAchievement, CooperativeStatistic, CooperativeAffiliation, LeadershipMessage, Person, Committee, Membership, Staff


class Command(BaseCommand):
    help = 'Migrate existing about data from home app to new about app'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Starting migration of about data...'))
        
        # Create company information
        company_info = CompanyInfo.objects.create(
            company_name="Bhanjyang Saving & Credit Cooperative Society Ltd.",
            company_name_nepali="भञ्ज्याङ बचत तथा ऋण सहकारी संस्था लिमिटेड",
            established_date=timezone.now().date().replace(year=2020),  # Adjust as needed
            registration_number="REG-2020-001",  # Update with actual number
            license_number="LIC-2020-001",  # Update with actual number
            address="Rupa Rural Municipality, Kaski, Nepal",
            phone="+977-61-XXXXXXX",  # Update with actual phone
            email="info@bhanjyang.coop.np",  # Update with actual email
        ...
        self.stdout.write(self.style.SUCCESS(f'Created company info: {company_info.company_name}'))
        ...
        timeline_events = [
            {
                ...
                'event_date': timezone.now().date().replace(year=2020, month=1, day=15),
            ...
            },
            {
                ...
                'event_date': timezone.now().date().replace(year=2020, month=6, day=1),
                'event_type': 'expansion'
            },
            {
                'title': '1000 Members Milestone',
                'description': 'Reached our first major milestone of 1000 active members.',
                'event_date': timezone.now().date().replace(year=2021, month=3, day=10),
        ...
        for event_data in timeline_events:
            event = CompanyTimeline.objects.create(
        ...
        for achievement_data in achievements:
            achievement = CompanyAchievement.objects.create(
        ...
        for stat_data in statistics:
            stat = CompanyStatistic.objects.create(
        ...
        for affiliation_data in affiliations:
            affiliation = CompanyAffiliation.objects.create(
        ...
        leadership_messages = [
            {
                ...
                'author_name': 'Chairman Name',  # Update with actual name
            ...
            },
            {
                ...
                'author_name': 'Manager Name',  # Update with actual name
        ...
        self.stdout.write(self.style.WARNING('Please update the sample data with actual information from your organization.'))
