...
class KYMSubmissionModelTest(TestCase):
    """Test suite for KYMSubmission model"""
    ...
    def setUp(self):
        ...
        self.submission = KYMSubmission.objects.create(
            ...
            citizenship_front=test_file,
            citizenship_back=test_file,
            passport_photo=test_file,
            address_proof=test_file,
            ip_address="192.168.1.1",
            user_agent="Test User Agent"
        )
    ...
    def test_str_representation(self):
        """Test string representation"""
        expected = f"John Doe - john@example.com ({self.submission.created_at.strftime('%Y-%m-%d')})"
        self.assertEqual(str(self.submission), expected)
    ...
    def test_status_choices(self):
        ...
        for status in statuses:
            submission = KYMSubmission.objects.create(
                full_name=f"Test User {status}",
                dob=date(1990, 1, 1),
                gender="male",
                marital_status="single",
                nationality="Nepali",
    ...
    def test_is_recent(self):
        ...
        # Old submission
        test_file = SimpleUploadedFile("test.pdf", b"file content", content_type="application/pdf")
        old_submission = KYMSubmission.objects.create(
            full_name="Old User",
            dob=date(1990, 1, 1),
            gender="male",
            marital_status="single",
    ...
    def test_reviewed_by_relationship(self):
        ...
        self.submission.reviewed_by = self.user
        self.submission.reviewed_at = timezone.now()
        self.submission.status = 'approved'
        self.submission.save()
        
        self.submission.refresh_from_db()
        self.assertEqual(self.submission.reviewed_by, self.user)
        self.assertIsNotNone(self.submission.reviewed_at)
    
    def test_ordering(self):
        """Test model ordering"""
        test_file = SimpleUploadedFile("test.pdf", b"file content", content_type="application/pdf")
        submission2 = KYMSubmission.objects.create(
        ...
        submissions = list(KYMSubmission.objects.all())
        # Should be ordered by -created_at (newest first)
        self.assertEqual(submissions[0], submission2)
        self.assertEqual(submissions[1], self.submission)
    ...
    def test_default_values(self):
        ...
        self.assertEqual(submission.province, "Gandaki Province")
