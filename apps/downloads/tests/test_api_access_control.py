...
from apps.downloads.security import AccessControlManager
...
class DownloadsAPIAccessControlTest(TestCase):
    ...
    def setUp(self):
        ...
        self.login_required_report = DownloadableFile.objects.create(
            title='Members Only Report',
            file=SimpleUploadedFile('members.pdf', b'members', content_type='application/pdf'),
            category=FileCategory.REPORT,
            is_active=True,
            requires_login=True,
        )
    ...
    def test_authenticated_non_staff_cannot_download_login_required_financial_report(self):
        """RPT files must require financial access even when requires_login=True."""
        can_download, reason = AccessControlManager.can_download_file(
            self.user,
            self.login_required_report,
        )
        ...
        download_url = reverse(
            'downloads_api:file-download',
            kwargs={'pk': self.login_required_report.pk},
        )
        response = self.client.post(download_url)
        self.assertEqual(response.status_code, 404)

    def test_authenticated_non_staff_list_excludes_financial_reports(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.list_url)
        ...
        titles = {item['title'] for item in response.data['results']}
        self.assertNotIn('Annual Report', titles)
        self.assertNotIn('Members Only Report', titles)
